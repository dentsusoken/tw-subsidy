            ...
            <ul className={"max-h-[148px] overflow-y-scroll"}>
                {
                    items && items.length > 0 ?
                        items.map((item, index) => (
                            <li key={index} className={"flex justify-between items-center h-11 pl-3 pr-4 my-1 w-full border-b text-sm"}>
                                ...
                                <span className={"text-xs mr-2 text-color-gray"}>{item.revokeStatus ? "発行済" : "取消済"}</span>
