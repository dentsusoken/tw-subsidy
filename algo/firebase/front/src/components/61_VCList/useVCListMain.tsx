...
const useVCListMain = () => {
  ...
  const [subsidyList, setSubsidyList] = useState<VCListItem[]>([]);
    ...
    (async () => {
      try {
        ...
        if (VCListGlobal.resident) {
          const residentList: VCListItem[] = await Promise.all(
            VCListGlobal.resident.map(async (item) => {
              const revokeStatus = await verifyVerifiableCredential(
                algod,
                item
              );
              return {
        ...
        if (VCListGlobal.account) {
          const accountList: VCListItem[] = await Promise.all(
            VCListGlobal.account.map(async (item) => {
              const revokeStatus = await verifyVerifiableCredential(
                algod,
                item
              );
        ...
        if (VCListGlobal.tax) {
          const taxList: VCListItem[] = await Promise.all(
            VCListGlobal.tax.map(async (item) => {
              const revokeStatus = await verifyVerifiableCredential(
                algod,
                item
              );
        ...
        if (VCListGlobal.subsidy) {
          const subsidyList: VCListItem[] = await Promise.all(
            VCListGlobal.subsidy.map(async (item) => {
              const revokeStatus = await verifyVerifiableCredential(
                algod,
                item
              );
