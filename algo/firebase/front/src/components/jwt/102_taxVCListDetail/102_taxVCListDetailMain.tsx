...
import { taxVCList2State } from '@/lib/mockApp/states';
...
const TaxVCListDetailMain = () => {
  ...
  const revoke = async () => {
    try {
      ...
      setIsRevoking(false);
      router.push(urls.taxVCListDone);
    } catch (e) {
      setIsRevoking(false);
      errorHandler(e);
    }
      ...
      <main className="bg-color-background">
        {vcInfo ? (
          <>
            ...
            {isTaxContent(vcInfo.vc.payload.vc.credentialSubject) ? (
              <TaxDetail input={vcInfo.vc.payload.vc.credentialSubject} />
            ) : null}
