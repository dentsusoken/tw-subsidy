...
import { residentVCList2State } from '@/lib/mockApp/states';
...
const ResidentVCListDetailMain = () => {
  ...
  const revoke = async () => {
    try {
      ...
      setIsRevoking(false);
      router.push(urls.residentVCListDone);
    } catch (e) {
      setIsRevoking(false);
      errorHandler(e);
    }
      ...
      <main className="bg-color-background">
        {vcInfo ? (
          <>
            ...
            {isResidentContent(vcInfo.vc.payload.vc.credentialSubject) ? (
              <ResidentDetail input={vcInfo.vc.payload.vc.credentialSubject} />
            ) : null}
