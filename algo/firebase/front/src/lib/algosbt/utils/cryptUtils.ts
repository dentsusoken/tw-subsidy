...
import { randomBytes, seedFromSecretKey } from './naclUtils';
...
export const encryptBySecretKey = (data: Uint8Array, secretKey: Uint8Array) => {
  const key = seedFromSecretKey(secretKey);

  return encrypt(data, key);
};

export const decryptBySecretKey = (
  encData: Uint8Array,
